...
def yolov5_decode_single_head(prediction, anchors, num_classes, input_dims, scale_x_y):
    ...
    assert input_dims[0] // grid_size[0] == input_dims[1] // grid_size[1], 'models stride mismatch.'
...
def yolov5_decode(predictions, anchors, num_classes, input_dims, elim_grid_sense=True):
    """
    ...
    for i, prediction in enumerate(predictions):
        results.append(yolov5_decode_single_head(prediction, anchors[anchor_mask[i]], num_classes, input_dims,
                                                scale_x_y=scale_x_y[i]))

    return np.concatenate(results, axis=1)


def yolov5_postprocess_np(yolo_outputs, image_shape, anchors, num_classes, model_image_size, max_boxes=100,
                         confidence=0.1, iou_threshold=0.4, elim_grid_sense=True):
    predictions = yolov5_decode(yolo_outputs, anchors, num_classes, input_dims=model_image_size,
