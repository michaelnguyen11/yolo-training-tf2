...
from yolov5.postprocess import yolov5_decode
...
def yolov5_loss(args, anchors, num_classes, ignore_thresh=.5, label_smoothing=0, elim_grid_sense=True,
    ...
    for i in range(num_layers):
        ...
        grid, raw_pred, pred_xy, pred_wh = yolov5_decode(yolo_outputs[i],
                                                        anchors[anchor_mask[i]], num_classes, input_shape,
                                                        scale_x_y=scale_x_y[i], calc_loss=True)
        ...
        if use_focal_obj_loss:
            ...
            confidence_loss = confidence_balance_weights[i] * sigmoid_focal_loss(true_objectness_probs,
                                                                                 raw_pred[..., 4:5])
        else:
            # confidence_loss = K.binary_crossentropy(true_objectness_probs, raw_pred[...,4:5], from_logits=True) * confidence_balance_weights[i]
            confidence_loss = confidence_balance_weights[i] * (
                        object_mask * K.binary_crossentropy(true_objectness_probs, raw_pred[..., 4:5],
                                                            from_logits=True) + \
