...
from tensorflow.keras.layers import Conv2D, DepthwiseConv2D, Add, Concatenate, MaxPooling2D, BatchNormalization, \
    Activation, UpSampling2D, ZeroPadding2D, Lambda
...
def bottleneck_csp_lite_block(x, num_filters, num_blocks, depth_multiple, width_multiple, shortcut=False,
    ...
    for i in range(num_blocks):
        y = compose(
            DarknetConv2D_BN_Swish(num_filters // 2, (1, 1)),
            # DarknetConv2D_BN_Swish(num_filters//2, (3,3)))(x)
            Depthwise_Separable_Conv2D_BN_Swish(filters=num_filters // 2, kernel_size=(3, 3),
                                                block_id_str=block_id_str + '_1'))(x)
        x = Add()([x, y]) if shortcut else y
...
def yolov5_spp_neck(x, num_filters):
...
def yolov5_predictions(feature_maps, feature_channel_nums, num_anchors, num_classes, depth_multiple, width_multiple,
                      with_spp=True):
    ...
    if with_spp:
        f1 = yolov5_spp_neck(f1, f1_channel_num)
...
def yolov5_lite_predictions(feature_maps, feature_channel_nums, num_anchors, num_classes, depth_multiple, width_multiple,
                          with_spp=True):
    f1, f2, f3 = feature_maps
    f1_channel_num, f2_channel_num, f3_channel_num = feature_channel_nums
    ...
    x1 = bottleneck_csp_lite_block(f1, f1_channel_num, 3, depth_multiple, width_multiple, shortcut=False,
                                   block_id_str='pred_1')
    ...
    x3_downsample = compose(
        ZeroPadding2D(((1, 0), (1, 0))),
        # DarknetConv2D_BN_Swish(f3_channel_num, (3,3), strides=(2,2)))(x3)
        Darknet_Depthwise_Separable_Conv2D_BN_Swish(f3_channel_num, (3, 3), strides=(2, 2), block_id_str='pred_3_2'))(
        x3)
    ...
    x2_downsample = compose(
        ZeroPadding2D(((1, 0), (1, 0))),
        # DarknetConv2D_BN_Swish(f2_channel_num, (3,3), strides=(2,2)))(x2)
        Darknet_Depthwise_Separable_Conv2D_BN_Swish(f2_channel_num, (3, 3), strides=(2, 2), block_id_str='pred_4_2'))(
        x2)
