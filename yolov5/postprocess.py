...
def yolov5_decode(feats, anchors, num_classes, input_shape, scale_x_y, calc_loss=False):
    ...
    box_wh = ((sigmoid_feats[..., 2:4] * 2) ** 2 * anchors_tensor) / K.cast(input_shape[..., ::-1],
                                                                            K.dtype(sigmoid_feats))
...
def yolov5_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape, scale_x_y):
    '''Process Conv layer output'''
    box_xy, box_wh, box_confidence, box_class_probs = yolov5_decode(feats,
                                                                    anchors, num_classes, input_shape,
                                                                    scale_x_y=scale_x_y)
    boxes = yolov5_correct_boxes(box_xy, box_wh, input_shape, image_shape)
    boxes = K.reshape(boxes, [-1, 4])
    # check if only 1 class for different score
    box_scores = tf.cond(K.equal(K.constant(value=num_classes, dtype='int32'), 1), lambda: box_confidence,
                         lambda: box_confidence * box_class_probs)
    box_scores = K.reshape(box_scores, [-1, num_classes])
    return boxes, box_scores


def yolov5_postprocess(args,
                       anchors,
                       num_classes,
                       max_boxes=100,
                       confidence=0.1,
    ...
    """Postprocess for YOLOv5 models on given input and return filtered boxes."""
    ...
    for l in range(num_layers):
        _boxes, _box_scores = yolov5_boxes_and_scores(yolo_outputs[l],
...
def batched_yolov5_boxes_and_scores(feats, anchors, num_classes, input_shape, image_shape, scale_x_y):
    '''Process Conv layer output'''
    box_xy, box_wh, box_confidence, box_class_probs = yolov5_decode(feats,
                                                                    anchors, num_classes, input_shape,
                                                                    scale_x_y=scale_x_y)
...
def batched_yolov5_postprocess(args,
                               anchors,
                               num_classes,
                               max_boxes=100,
                               confidence=0.1,
                               iou_threshold=0.4,
                               elim_grid_sense=True):
    """Postprocess for YOLOv3 models on given input and return filtered boxes."""
    ...
    box_scores = []
    for l in range(num_layers):
        _boxes, _box_scores = batched_yolov5_boxes_and_scores(yolo_outputs[l],
...
def yolov5_correct_boxes(box_xy, box_wh, input_shape, image_shape):
    '''Get corrected boxes'''
