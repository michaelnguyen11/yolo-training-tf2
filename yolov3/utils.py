...
def image_preprocess(image, target_size, gt_boxes=None):
    ...
    scaled_w, scaled_h = int(scale * image_w), int(scale * image_h)
    image_resized = cv2.resize(image, (scaled_w, scaled_h))

    image_padded = np.full(shape=[target_h, target_w, 3], fill_value=128.0)
    dw, dh = (target_w - scaled_w) // 2, (target_h - scaled_h) // 2
    image_padded[dh:scaled_h + dh, dw:scaled_w + dw, :] = image_resized
...
def postprocess_bboxes(pred_bbox, image_shape, input_size, score_threshold):
