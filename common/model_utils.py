...
def add_metrics(model, metric_dict):
    '''
    add metric scalar tensor into models, which could be tracked in training
    log and tensorboard callback
    '''
    for (name, metric) in metric_dict.items():
        # seems add_metric() is newly added in tf.keras. So if you
        # want to customize metrics on raw keras models, just use
        # "metrics_names" and "metrics_tensors" as follow:
        #
        # models.metrics_names.append(name)
        # models.metrics_tensors.append(loss)
...
def get_pruning_model(model, begin_step, end_step):
    ...
    if tf.__version__.startswith('2'):
        # models pruning API is not supported in TF 2.0 yet
        raise Exception('models pruning is not fully supported in TF 2.x, Please switch env to TF 1.x for this feature')
...
def get_lr_scheduler(learning_rate, decay_type, decay_steps):
    ...
    if decay_type == None:
        ...
        lr_scheduler = PolynomialDecay(initial_learning_rate=learning_rate, decay_steps=decay_steps,
...
class ExponentialMovingAverage(object):
    """
    Apply exponential moving average on models weights
    Reference:
    ...
    Usage:
            models.compile(...)
            ...
            EMAer = ExponentialMovingAverage(models)
            EMAer.inject()
            ...
            models.fit(x_train, y_train) #keep on training
    """
    ...
    def initialize(self):
        """
        initialize ema_weights with origin models weights
        """
    ...
    def apply_ema_weights(self):
        """
        store origin models weights, then apply the ema_weights
        to models
        """
        self.old_weights = K.batch_get_value(self.model.weights)
