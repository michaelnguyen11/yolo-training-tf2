...
from yolov5.model import get_yolov5_train_model
from yolov5.data import yolov5_data_generator, Yolov5DataGenerator
...
def main(args):
    ...
    # train_data_generator = Yolov5DataGenerator(dataset[:num_train], args.batch_size, input_shape, anchors, num_classes, args.enhance_augment, rescale_interval, args.multi_anchor_assign)
    # val_data_generator = Yolov5DataGenerator(dataset[num_train:], args.batch_size, input_shape, anchors, num_classes, multi_anchor_assign=args.multi_anchor_assign)

    tiny_version = False

    # prepare online evaluation callback
    if args.eval_online:
        eval_callback = EvalCallBack(args.model_type, dataset[num_train:], anchors, class_names, args.model_image_size,
                                     args.model_pruning, log_dir, eval_epoch_interval=args.eval_epoch_interval,
    ...
    if args.model_pruning:
        pruning_callbacks = [UpdatePruningStep(), PruningSummaries(log_dir=log_dir, profile_batch=0)]
    ...
    if args.gpu_num >= 2:
        ...
        with strategy.scope():
            ...
            model = get_yolov5_train_model(args.model_type, anchors, num_classes, weights_path=args.weights_path,
                                           freeze_level=freeze_level, optimizer=optimizer,
                                           label_smoothing=args.label_smoothing, elim_grid_sense=args.elim_grid_sense,
                                           model_pruning=args.model_pruning, pruning_end_step=pruning_end_step)
        ...
        model = get_yolov5_train_model(args.model_type, anchors, num_classes, weights_path=args.weights_path,
                                       ...
                                       elim_grid_sense=args.elim_grid_sense, model_pruning=args.model_pruning,
    ...
    model.fit_generator(
        yolov5_data_generator(dataset[:num_train], args.batch_size, input_shape, anchors, num_classes,
        ...
        validation_data=yolov5_data_generator(dataset[num_train:], args.batch_size, input_shape, anchors, num_classes,
    ...
    if args.decay_type or args.average_type:
        ...
        optimizer = get_optimizer(args.optimizer, args.learning_rate, average_type=args.average_type,
                                  decay_type=args.decay_type, decay_steps=decay_steps)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', type=str, required=False, default='yolo5_mobilenetv2_lite',
                        help='YOLO model type: yolo5_mobilenetv2_lite/yolo5_small_darknet/yolo5_medium_darknet/..., default=%(default)s')
    parser.add_argument('--anchors_path', type=str, required=False,
                        default=os.path.join('configs', 'yolov5_anchors.txt'),
    ...
    parser.add_argument('--average_type', type=str, required=False, default=None,
